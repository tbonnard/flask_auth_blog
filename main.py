    ...
    if form.validate_on_submit():
        ...
        if user:
            if check_password_hash(user.password, form.password.data):
                ...
                flash("pwd not correct")
                return redirect(url_for('login'))
        else:
            flash("Email not in DB")
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash('You must be logged in to add a new comment')
            return redirect(url_for('login'))
        new_comment = Comments(body=form.body.data, date=date.today().strftime("%B %d, %Y"), author=current_user, post=requested_post)
